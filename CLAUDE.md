...
- `turmas.js` — CRUD turmas/turma_disciplinas, matrículas (admin: matricular/decidir; aluno: solicitar/cancelar), vínculo N-N do conteúdo pelos dois lados: aula→turma (setTurmasDaAula/DoSimulado, turmasDaAula/DoSimulado) e turma→aulas (aulasDaTurma/setAulasDaTurma, simuladosDaTurma/setSimuladosDaTurma — modal "Conteúdo da turma" na CentralMatriculas), buscarTurmaComConteudo(id), progressoTurma(id) (questões respondidas ÷ total do conteúdo da turma), contarSolicitacoesPendentes.
