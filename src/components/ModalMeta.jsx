...
function distintos(facetas, campo, rel, pred = () => true) {
  const m = new Map()
...
// Modal de objetivo (banca / disciplinas / assuntos) + metas
export default function ModalMeta({ cfgInicial, facetas = [], onFechar, onSalvar }) {
  ...
  const bancas = useMemo(() => distintos(facetas, 'banca_id', 'bancas'), [facetas])
  const facBanca = useMemo(() => facetas.filter(q => !banca || String(q.banca_id) === String(banca)), [facetas, banca])
  const disciplinas = useMemo(() => distintos(facBanca, 'disciplina_id', 'disciplinas'), [facBanca])
  const discsSel = new Set(Object.keys(porDisc))
  const assuntosDisp = useMemo(
    () => distintos(facBanca, 'assunto_id', 'assuntos', q => discsSel.size === 0 || discsSel.has(String(q.disciplina_id))),
    [facBanca, Object.keys(porDisc).join(',')])
  ...
  function salvar() {
    // mantém só assuntos ainda válidos no escopo
    const validos = new Set(assuntosDisp.map(a => a.id))
    const assuntosLimpo = [...assuntos].filter(a => validos.has(a))
    onSalvar({
      metaDiaria: Number(metaDiaria) || 0, metaDias: Number(metaDias) || 0,
      objetivo: { banca_id: banca || null, assuntos: assuntosLimpo },
      porDisciplina: { ...porDisc },
    })
  }

  const comGoal = Object.keys(porDisc)
    ...
    <div className={styles.overlay} onClick={onFechar}>
      <div className={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.secao}>
          ...
          <span className={styles.label} style={{ marginTop: 12, display: 'block' }}>Disciplinas</span>
          ...
          {assuntosDisp.length > 0 && (
            <>
              <span className={styles.label} style={{ marginTop: 12, display: 'block' }}>
                Assuntos (opcional{discsSel.size ? ' — dentro das disciplinas escolhidas' : ''})
              </span>
              <div className={styles.chips}>
                {assuntosDisp.map(a => (
                  <button key={a.id} type="button"
                    className={`${styles.chip} ${assuntos.has(a.id) ? styles.chipOn : ''}`}
